...
The value aggregation could also reveal the integrated value for scientific facilities, experiemntal techniques, and sensor technologies.
...
\begin{itemize}
	\item In the first period we will concentrate on FPGA development for two core LCLS-II projects, 1) interpreting electron time of flight spectra for the quintessential example of a fire hose data source, the so-called CookieBox detector, and 2) optical image segmentation and pattern recognition for 2D area detectors in the visible range for the 2D-TimeTool delar sorting diagnostic.
	...
	\itme We will also generate unique data and algorithm IDs that chain together to be included in the corresponding the data header.
...
Pre-analysis for images---so called ``featurization''---reduces typically MPix images into a lower dimensional set of features before presenting to a machine learning inference model.
This featurization will occur in custom logic blocks in the frame-grabber FPGAs and then transfer directly to a more ML tailored inference chip such as a secondary inference accelerator \cite{waveCGRA,CGRAreview,seaofcores}.
...
\begin{itemize}
	\item Domain specific interpretation of raw data via Edge AI in multiple format sensor FPGAs, e.g. waveform digitizers and high frame rate imaging detectors including the EPix family of x-ray area detectors.
	\item ASIC ID and FPGA firmware fingerprint used for unique identification in the data header.
	\item Dynimic analysis chain that is direcly encoded into the identification ledger of record.
	\item Demonstration of data ID used to imbue data and algoritm with quantitative value that scales propotional to the scientific use of data and algorithms.
