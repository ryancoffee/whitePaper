...
One of the most significant challenges for data distillation in sensor firmware at light sources is the is the weekly cycle of experimental reconfiguration.
Each experiment brings its own domain specific conventional algortihms or custom machine learned inference models.
These models will be flashed to custom logic regions in detector-side Field Programmable Gate Arrays (FPGAs).
The computational complexity of optimizing FPGA routing \cite{FPGAcomplexity,FPGAgraph} is notoriously challenging and its $np$-completeness lies at the heart of why mdular code that easily compiles for conventional CPUs can fail in surprising ways when compiled to firmware for FPGA. 
Our long experience with traditional imperative CPU programming has encouraged the paradim of designing the control flow of functions or procedures by stringing existing algorithmic modules.
In a CPU, if a module implimentation changes, it should not affect the stability or function of its interacting modules.
Declarative programming is differnt in that it focusses exclusively on the logical result of a computational task, not the control flow and as such is more ameanable to compilation to FPGA and other non-CPU hardware as well as better exploiting parallelism \cite{Lloyd1994}.
...
In such an on-sensor yet heterogeneous ecosystem one would leverage a hardware palette as envisioned in Fig.~\ref{fig;:EdgeFlow} whereby FPGA usage is likely concentrated at the earliest stages in data transformation, followed by deeper complex decision models in GPU chips with final data routing decisions likely controlled by the on-borad CPU.
\begin{figure}
