...
The drive in industry for intelligent sensors is is not mirrored within DOE-BES facility portfolio, it is amplified by many orders.
...
We propose a shift to processing information at the point of production, in the experimental sensors themselves, that provides many orders of magnitude in data compression while preserving both validation and forensic reconstruction of decision chains.
Conintuing with the HDR photoraphy analogy, we aim to encode the exposure set and image interlacing information into the header of the final HDR image out along with the firmware version and a pointer to the cloud hosted algortihm.
...
One rather novel challenge for Department of Energy user facilities is that unlike the HDR analogy, data distillation in sensor firmware at a beamline must accommodates a weekly cycle of experimental reconfiguration.
Each experimental group will bring its own domain specific algorithms or custom machine learned inference models.
...
An on-sensor heterogeneous ecosystem could thus leverage domain specific portfolio of chip architectures as envisioned in Fig.~\ref{fig;:EdgeFlow} whereby FPGA usage is likely concentrated at the earliest stages in data transformation followed by deeper decision models in GPU or other inference specific ASIC chips with the final data routing decisions being controlled by on-borad ARM CPUs.
HERE HERE HERE HERE
\begin{figure}
...
The need for sensor data fustion a heterogeneous environment is not unique to DOE facilities like the LCLS-II where come sensors will report at the 1MFps machine rate while  others will report at a much slower, pre-scaled, $\sim$100Hz "validation" rate.  
In the agriculture industry, aerial drones can provide hourly hyper-spectral imaging of croplands whereas soil testing occurs at most frequency daily or weekly.
Multiple duty cycle sensor reporting is a pervasive problem of data fusion that will only grow more complicated as the data rates increase.
Again, provenance must be fundamentally included together with the data events since the event alignment for fusion of asynchronous sensors will nevertheless require forensic reconstruction.
