...
There is also a growing portfolio of newly enabled experimental techniques such as time-resolved x-ray holography imaging \cite{tais} of biomolecular systems and time-resolved dark field x-ray microscopy \cite{leora} that interrogate nano- and meso-scopic movies of the action of photochemistry and emergent materials properties.
...
\paragraph{from Audrey}
%The Linac Coherent Light Source II (LCLS-II) holds great promise to study critical rare events such as the onset of battery failure as well as the random cancer-inducing DNA mutations during DNA replication. 
%Similarly, LCLS-II will enable to study the atomic motions and chemical reactions happening during gene therapy. 
...
This large data paradigm is not a problem exclusive to DOE light sources like LCLS-II and the Advanced Photon Source Upgrade (APS-U); it has become a critical issue for several large scientific experiments as well as for industrial applications with the ongoing growth of the Industrial Internet of Things (IIoT)\cite{Gartner2018,NetworkWorld2019}.

In the context of DOE's flagship facilities, LCLS-II data rates could grow to velocities beyond 1~TB/s with similar challenges following shortly thereafter at APS-U.
At this data velocity, the data volume for a single 12 hour shift is equivalent to producing 28 years worth of HD video \cite{HowBigVideo} requiring just shy of 1/2 M\$ in storage assuming \$0.01/GB \cite{CostPerTB}\footnote{This only considers hardware costs. Actual costs including power, space and personnel are much higher (estimated at \$30 per GB per month) and accumulate over time.}.
The data production is projected to increase further as we advance towards the goal of running the planned multi-megapixel camera at a MHz full frame readout by 2028.
...
We propose the use of AI at or near the detector --- at the edge --- to analyze, veto and reduce the incoming data to a useful stream of information. 
Although Edge AI growth is picking up speed, the DOE has a unique need for high velocity, low latency, easily adaptive and flexible Edge AI.
In addition to reducing data volumes and throughput, the ability to move some of the CPU/GPU data analysis into hardware and firmware is an enabling step to dynamic experiments which require low latency.
With fast analysis, feedback can be provided to the light source and the detector to dynamically adjust experiment parameters. 
Furthermore, each week new users mount experiments with vastly different requirements and objectives which precludes a single static data acquisition system. 
We require a flexible acquisition system whereby our scientific instruments directly record actionable information specific to the experiment, not raw data.
 This acquisition system must be in a form that can elegantly handle the frequent---weekly or daily---re-definition of ``information''.
The process will require real-time low latency streaming inference engines that will dynamically route data directly through the different Edge AI hardware along the data acquisition chain, be it FPGA, GPU, CPU, or other emerging hardware~\cite{edgetpu_benchmarks,edgetpu,waveCGRA,CGRAreview,seaofcores}.
There exists a concomitant need for data and model provenance tracking for the dynamic Edge AI systems that are actively manipulating the data in flight to storage.
Both data and model provenance and verification must therefore be baked directly into the adaptive Edge AI paradigm for DOE's user facilities.
...
The provenane ledger continues to track and autoincrement a use metric according to the derived scientific value of both data and algorithm.
This ``value aware'' ledger can then be used for an automated dynamic data retention policy whereby lifetime in archive scales proportial to the evolveing scientific value of a particular data set and/or algorithm.
