...
This would potentially harm the inferred results of data analysis and potentially mis-lead technological development.
Heading off such a threat by cooking data provenance and verification directly into the an adaptive Edge Computing paradigm is the route we propose to demonstrate, initially with a few crisp examples from DOE's user facilities, and then with expanded application to the broader industrial and health sector use cases.

\paragraph{Proposed solution}
Blockchain is a form of Distributed Ledgerr Technology (DLT) that is opening a new paradigm for data movement with collaborative verification and storage of transactional data.
We will initially focus on field programmable gate arrays (FPGAs) 
%and application specific integrated circuits (ASICs) 
at the sensor and quickly move to also include emerging Edge Computing microchips such as the EdgeTPU \cite{edgetpu_benchmarks,edgetpu}, coarse-grained reconfigrurable arrays \cite{waveCGRA,CGRAreview}, ``sea-of-cores'' \cite{seaofcores} and others.
%AI accelerating chips. at the convergence of multiple sensors.
The chips will simultaneously execute the Edge inference models while updating the blockchain for that unit of information.
The sequence of block added to the chain will fundamentally bind a fingerprint of the inference model to the information being produced, and thus create a uniquely identifiable transaction record as depicted in Fig.~\ref{fig::EdgeFlow}.
...
The perpetual ledger record allows for dynamic retention that scales with the scientific or social value derived from the data and/or algorithm.
...
In the first period we will concentrate on FPGA development for interpreting electron time of flight spectra for one of DOE's quentessential examples of a firehose data source, the so-called CookieBox detector for LCLS-II.
...
%\emph{Year 2: \$1M}
Folloing this initial demonstration, we will work together with the high frame rate camera producer and the associated frame-grabber supplier to demonstrate the same transaction ledger Edge Computing paradigm for image transformation.
The pre-analysis for images os often called ``featurization'' since it reduces typically MPix images into a mor eappropriate set of features before presenting to a machine learning inference model.
The featureization will occur in custom logic blocks in the framegrabber FPGAs and then transfer directly to a more ML tailored inference chip such as a secondary inference accelerator \cite{waveCGRA,CGRAreview,seaofcores}.
...
At this stage we will also demonstrate the value-binding for data such that the beneficial outcome of the data and inference models can proportionatly increment the ``retention value'' of the data element.
...
We attempt to intercept data and make modifications ``in-flight'' to show how it would render the data useless, spoiling the decription algorithm.
...
The ledger record will record interactions with the data in the training set such that attemts to use or alter the data will also appear in the transaction record.
Since the transaction record and training set fingerprints are used as so-called ``salt'' in the data encryption process, altered data would be rendered non-decryptable.
Beyond nuclear security, the sharing of anonamized health data can be more securely controlled and therefore more confidently shared with exclusive trusted partners.
%and rendered useless if alteration is attempted.
In the same way as the scientific data is itself imbued with folow on value, so too with medical data; the originator of data could recieve direct benefit for sharing, a benefit that could scale in proportion to the market value of output that derived from thier data.
