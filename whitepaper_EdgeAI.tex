...
% 88GB/hr of movies = 11.4 hr of HD movies per second of LCLS-II ... 2454545 hours of movie per 5 shift experiment = 280 years of movie per experiment, 56 years per shift... thats a lot of grad students.
%
%that will quickly exceed network and storage capacity, thus providing early insight into the next decade of IIoT challenges.
This data liability is not unique of DOE facilities, but for us it is of such scale and immediacy that we must reformulate traditional data acquisition (DAQ) and Cloud hosted data mining into a concept of information acquisition (IAQ) whereby we direcly record actionable information, not raw data, a form that can flexiply handle the redefinition of ``information'' every week---or more likely every shift.
...
When chips hit the physical limitation of a few GHz (lower: \textcolor{purple}{$\bullet$}), industry immediately responded with the multi-threading paradigm (upper: \textcolor{green}{$\circ$}).
Physical limits are always circumvented by human creativity and the limits of this decade are dat volume and velocity and humanities response is Edge Computing \cite{thetech,NetworkWorld2019b}.
...
Although the Edge Computing paradigm has already begun, the DOE need for rapidly adaptive and flexible Edge Computing raises a simultaneous need data provenance tracking.
%Security that data is not being manipulated ``in-flight'' is certainly critical to autonomous vehicles \cite{brookings2019}, but one can also imagine how bad actors might mettle with or inject obfuscation into shared scientific data sets.
%This would potentially harm the inferred results of data analysis and potentially mislead technological development.
Data provenance and verification must be baked directly into the adaptive Edge Computing paradigm for DOE's user facilities.
%, and then with expanded application to the broader industrial and health sector use cases.

\paragraph{Proposed solution}
We take inspiration for out Edge Computing from Nature's own ``wet ware'' solution in the human eye.
Much of human vision processing occurs directly in the retna, in the Retinal Ganglion Cells (RGC), well before passage via axons into the occipital lobe for deeper neural network inference.
Decicive actions are triggered in the cornea, stabilizing eye movements, attention and anomoly actions, these all can precede even the occipital awareness of the formulated image content.
We propose a similarly function for our scientific sensors.
We will use Field Programmable Gate Arrays (FPGAs) and other emerging Edge hardware \cite{edgetpu_benchmarks,edgetpu,waveCGRA,CGRAreview,seaofcores} both directly at the sensor and also in directly connected intermediate computing nodes to host flexibly and dynamically adaptive algorithms and Machine Learning (ML) inference models to extract contextually relevant information for further propagation down the analysis chain toward eventual storage to memory.
The FPGAs themselves will use the in-flight information to adapt the analysis chain dynamically, stitching into the data header not only the data ID, but also the shot-to-shot algorithm ID.
