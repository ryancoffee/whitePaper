...
\title{EdgeAI for an Intelligent and Secure Scientific Information Maketplace}
\author{Ryan Coffee}
...
\begin{wrapfigure}[33]{r}{.4\linewidth}
	...
	\caption{
		...
		Data adapted from Ref.~\cite{MicroprocessorTrendData}. The mid-2000s saw that clock and thermal limitations (lower) triggered the multi-threadding paradigm.
...
\end{wrapfigure}
Processing data at the point of generation, at the sensor or cluster of sensors, is key to the future of Industrial Internet of Things (IIoT) \cite{Gartner2018,NetworkWorld2019}, also refered to as Industry 4.0.
...
%It is no suprise that, respecting Nyquist waveform sampling, we hit a physical clock limitation in the vicinity of a few GHz (lower: \textcolor{purple}{$\bullet$}).
When we hit the physical limitaiton of a few GHz (lower: \textcolor{purple}{$\bullet$}), industry immediately responded with the multi-threading paradigm (upper: \textcolor{green}{$\circ$}).
The computing bottleneck of this decade is now data transfer to the compute engine, but the response to this limitation is already being formulated as Edge Computing \cite{thetech,NetworkWorld2019b}.
Much like multi-threading allowed continued exponential growth in the data center, Edge Computing will enable continued exponential growth of distributed sensors for autonomous decisions and information compression.
...
Security that data is not being manipulated ``in-flight'' is certainly critical to autonomous vehicles, but one can also imagine how bad actors might mettle with or contribute obfuscation into shared scientific data sets.
This would potentially harm the inferred results of data analysis, potentially mis-leading technological development.
Heading off such a threat by cooking data provenance and verification directly into the Edge Computing paradigm by capitolizing on recent developments in Blockchain Technology is the route we propose to demonstrate, initially with a few crisp examples from the DOE's user facility compex, and then expanded application to broader industrial and health sector use cases.

\paragraph{Proposed solution}
Blockchain is a form of Distributed Ledgerr Technology (DLT) that is opening a new paradigm for data movement and the collaborative verification and storage of transactional data.
We intend to use field programmable gate arrays (FPGAs) and application specific integrated circuits (ASICs) at the sensor along with emerging Edge Computing microchips such as the EdgeTPU \cite{edgetpu_benchmarks,edgetpu}, coarse-grained reconfigrurable arrays \cite{waveCGRA,CGRAreview}, the ``sea-of-cores'' \cite{seaofcores} and other AI accelerating chips at the convergence of multiple sensors.
These chips will simultaneously execute Edge Computing inference models, fundamentally bind a fingerprint of the model to the information being produced, and update the blockchain for that unit of information regarding the algorithm execution chain as a uniquely identifiable transaction record.
We plan to enable encryption that uses this transaction ledger as the encryption ``salt'' such that veracity can be bound to the data and its retrieval; one must match the ledger with the datum to achieve decryption.
The perpetual ledger record allows for dynamic retention that scales with the scientific or social value derived from the combined data and algorithm.
Not only does this inform the data centers of which data and algorithms can be safely discarded and which should be retained for continued value extraction, but also this data value can be aggregated to show value metrics for specefic scientific facilities or sensor technologies.
...
In the first period we will concentrate on FPGA development for interpreting electron time of flight spectra for a quentessential example of DOE's firehose data source, the so-called CookieBox detector for LCLS-II.
We will demonstrate multiple pre-analysis algorithms in firmware of the digitizer FPGA where each algorithm is developed for a specific application of the detector and therefore represent unique domain specific algorithms that each effectively re-brand the detector for fundamentally different interpretations of the recorded analog waveforms.
We will demonstrate that this same FPGA can generate the unique id that also binds to the corresponding algorithm, both of which produce a fingerprint to a unique data identifier.
This identifier will form the initial block in the transaction chain, thus initiating the hyperledger record.
The deeper analysis in GPU and CPU that follow this pre-processing will then be treated as subsequent blocks, transactions that will be added to the ledger.

%Also in Year 1 we will design an RF ASIC that pre-conditions the analog waveform in order to optimally distribute the uniform sampling distribution of the ADC convertes in the digitizers.

%\emph{Year 2: \$1M}
In Year 2 we will work together with Euresys Inc. to demonstrate image transformation, also called ``featurization,'' as a preliminary step before presenting the information to a machine learning inference model.
The featureization will occur in custom logic blocks in the framegrabber FPGAs and then transfer directly to a more ML tailored inference chip such as a secondary FPGA, CGRA, or FractalCore.
...
These too will be treated as transaction that will be added to the blockchain.
We will also demonstrate the value metric binding to data such that the outcome of e.g. resulting subsequent inference models can proportionatly increment the ``retention value'' of the data element.

Also in year 2, we will design an RF analog ASIC that will work together with an ACD chip and FPGA on a custom board that allows for the full pre-analysis of all 16 data channels of the CookieBox to feed directly into an inference engine that bypasses the PCIe bus.


%\emph{Year 3: \$1M }
In Year 3 we will work together with Vision Research and the various chip makers to integrate what had previously been hosted on the framegrabber into the camera electronics.
We will also help define the EdgeAI chip integration standard for industrial diagnostic sensors and medical electronics, to be hosted as embedded daughter cards.
We will demonstrate that intercepting data and making modifications ``in-flight'' will render the data useless as it will spoil the decription algorithm.
By year 3 we will prepare demonstration examples for exansion to industry and medical sensor domains.

\paragraph{Broader applications}
With an increasing reliance on machine learning trained models for radio imaging applications in nuclear security comes the risk that such models can be nefariously altered.
This alteration can come from tampering with the models themselves, or less directly by injecting falsified training data into the training pool.
By using an algorithm fingerprint as a so-called ``salt'' in the data encryption process, we can render altered data useless; it will be non-decryptable.
Furthermore, the ledger record will record interactions with the data such that attemts to use or alter the data will also appear in the transaction record.
Beyond nuclear security, the sharing of anonamized health data can be controlled and rendered useless if alteration is attempted.
In the same way as scientific value is imbued on the data set via the unique identifiers and transaction logs, so too with medical data the originator of the data could recieve direct benefit for sharing that information, a benefit that could scale in proportion to the market value of output that derived from thier data.
We note that the medical sensor portfolio is tremendous, it could even extend application to secure transfer and value tracking of in-home sensors for e.g. elder care, nutrition, and diabetes.
...
\begin{itemize}
	...
	\item Transation ledger based on block chain and distributed ledger technology that includes CPU model and code and GPU instructions as fingerprints written into the transaction record.
	\item Imbue original data with a value metric that scales with the successful output from subsequent data transactions.
